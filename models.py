...
from utils import dump_embedding, euclidean_dist, output_conv_size
    ...
    def __init__(self,
        ...
        if True:
            ...
            if self.use_rp:
                ...
                self.conv_1 = nn.Conv1d(self.channel,
                                        filters[0],
                                        kernel_size=kernels[0],
                                        dilation=dilation,
                                        stride=1,
            ...
            self.conv_2 = nn.Conv1d(filters[0],
                                    filters[1],
            ...
            self.conv_3 = nn.Conv1d(filters[1],
                                    filters[2],
            ...
            if self.use_cnn:
                ...
                for i in range(len(filters)):
                    conv_size = output_conv_size(conv_size, kernels[i], 1,
                                                 paddings[i])
                fc_input += conv_size
        ...
        for i in range(len(layers) - 2):
            self.mapping.add_module("fc_" + str(i),
                                    nn.Linear(layers[i], layers[i + 1]))
            self.mapping.add_module("bn_" + str(i),
                                    nn.BatchNorm1d(layers[i + 1]))
            self.mapping.add_module("relu_" + str(i), nn.LeakyReLU())

        # add last layer
        self.mapping.add_module("fc_" + str(len(layers) - 2),
                                nn.Linear(layers[-2], layers[-1]))
        if len(layers) == 2:  # if only one layer, add batch normalization
            self.mapping.add_module("bn_" + str(len(layers) - 2),
        ...
        if self.use_att:
            ...
            for _ in range(nclass):
                ...
                att_model = nn.Sequential(nn.Linear(layers[-1], att_dim),
                                          nn.Tanh(), nn.Linear(att_dim, 1))
