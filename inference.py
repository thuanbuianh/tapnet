...
file = st.file_uploader(f'Please upload {dataset_option} test file (in csv format)', type='csv')

model_settings = {'ArticularyWordRecognition': {'nfeat':9,
                                                'len_ts':144,
                                                'layers':[500,300],
                                                'nclass':25,
                                                'dropout':0.5,
...
model_path = './models'
model.load_state_dict(torch.load(f'{model_path}/{dataset_option}.pth', map_location=torch.device('cpu')))
model.eval()

features, labels, idx_train, idx_test, le = load_test_ts(file, dataset_option)
